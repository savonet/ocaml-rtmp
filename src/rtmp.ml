...
let chunkify cnx ~chunk_stream_id ~timestamp ~message_type_id ~message_stream_id
    data =
  let data = Bytes.unsafe_of_string data in
  let message_length = Bytes.length data in
  chunk_header0 cnx.socket ~chunk_stream_id ~timestamp ~message_length
    ~message_type_id ~message_stream_id;
...
let control_message cnx message_type_id payload =
  ...
  (* TODO *)
  chunk_header0 cnx.socket ~chunk_stream_id:2 ~timestamp
...
let user_control cnx t data =
  ...
  assert (String.length data + 2 <= cnx.chunk_size);
  (* TODO *)
...
let command cnx ?(timestamp = now cnx) ?(message_stream_id = Int32.zero) name
    transaction_id params =
  Printf.printf "\nCOMMAND: %s\n%!" name;
  let data =
    Amf.encode_list ([Amf.String name; Amf.int transaction_id] @ params)
  in
  chunkify cnx ~chunk_stream_id:3 ~timestamp ~message_type_id:0x14
    ~message_stream_id data

let audio cnx ?(timestamp = now cnx) ?(message_stream_id = Int32.zero) data =
  chunkify cnx ~chunk_stream_id:4 ~timestamp ~message_type_id:0x08
    ~message_stream_id data

let video cnx ?(timestamp = now cnx) ?(message_stream_id = Int32.zero) data =
  chunkify cnx ~chunk_stream_id:5 ~timestamp ~message_type_id:0x09
    ~message_stream_id data

let data cnx ?(message_stream_id = Int32.zero) amf =
  let timestamp = now cnx in
  let data = Amf.encode_list amf in
  chunkify cnx ~chunk_stream_id:6 ~timestamp ~message_type_id:0x12
    ~message_stream_id data
...
let has_message cnx = not (Queue.is_empty cnx.messages)
    ...
    match msg.message_type_id with
      ...
      | 0x14 -> (
          ...
          match Amf.get_string amf.(0) with
            | "connect" -> `Command (tid, `Connect)
            | "createStream" -> `Command (tid, `Create_stream)
            | "deleteStream" ->
